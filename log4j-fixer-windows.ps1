...
begin
{
   ...
   $TempArchive = ('{0}\{1}\tmp.zip' -f ($WorkDirectory), ($TempDirectory))
   ...
   $LogFile = ('{0}\{1}-Log4j-Scann-Results-{2}.txt' -f ($WorkDirectory), ($env:COMPUTERNAME), (Get-Date -Format 'MM-dd-yyyy_HH-mm-ss'))
}
...
process
{
   ...
   foreach ($FixedDisk in $AllFixedDisks.DriveLetter)
   {
      ...
      Get-ChildItem -Path ('{0}:\' -f $FixedDisk) -Filter 'log4j*.jar' -Recurse -File -ErrorAction SilentlyContinue | ForEach-Object -Process {
         ...
         Copy-Item -Path $JARArchiveFile -Destination $TempArchive

         # Uncompress the JAR archive
         Expand-Archive -Path $TempArchive -DestinationPath $UnpackedDirectory
...
#         }
         ...
         if ($unsafe)
         {
            ...
            Get-ChildItem -Path $UnpackedDirectory -Filter 'JndiLookup.class' -Recurse -ErrorAction SilentlyContinue | ForEach-Object -Process {
               ...
               copy-item -Path $TempArchive -Destination "$WorkDirectory\$($JARArchiveFile -replace '\\','-' -replace ':','_')"
            ...
            if ($AutoFix)
            {
               ...
               (Compress-Archive -Path ('{0}\*' -f ($UnpackedDirectory)) -DestinationPath $FixedArchive -Force -Confirm:$false -ErrorAction SilentlyContinue)

               # Restore the new JAR archive
               (Copy-Item -Path $FixedArchive -Destination $JARArchiveFile -Force -Confirm:$false)

               # cleanup
               $null = (Remove-Item -Path $FixedArchive -Force -Confirm:$false -ErrorAction SilentlyContinue)
            }
         }

         # Further cleanup
         $null = (Remove-Item -Path $TempArchive -Force -Confirm:$false -ErrorAction SilentlyContinue)
...
end
{
   ...
   # save/zip original files for if something breaks to saved_files.out
   Compress-Archive -DestinationPath "$WorkDirectory\saved_files.zip" -Path "$WorkDirectory\*.jar" -CompressionLevel Fastest
   move-item "$WorkDirectory\saved_files.zip" "$WorkDirectory\saved_files.out"
   remove-item "$WorkDirectory\*.jar" -Force
}